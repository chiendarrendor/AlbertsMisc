...
const LDReturn& LDist::LevenshteinDistance(const std::string& s1,int s1idx,const std::string& s2,int s2idx) const
{
  ...
  if (s1.length() == 0)
  {
    LDReturn ldr(s2.length() > 0 ? -m_score.GetGapPenalty() : 0);
  ...
  if (s2.length() == 0)
  {
    LDReturn ldr(s1.length() > 0 ? -m_score.GetGapPenalty() : 0);
  ...
  int od1 = opt1.distance - (opt1.deltas[0] == 'D' ? 0 : m_score.GetGapPenalty());
  int od2 = opt2.distance - (opt2.deltas[0] == 'I' ? 0 : m_score.GetGapPenalty());
