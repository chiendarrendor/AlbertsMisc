...
sub Astar($$)
{
  ...
  if ($grade >= $initial->WinGrade())
  {
      my $p = MakePath($initial,1);
      push @result,$p;
      PrintPath($p,$doprint);
      return @result;
  }
...
#            ", num unique states: ",scalar(keys %states),"\n";
	...
	$grade = $nextstate->Grade();
	    ...
	    my $p = MakePath($nextstate,1);
	    push @result,$p;
	    PrintPath($p,$doprint);
	    return @result if @result >= $rcount;
	    next;
        } 
...
#          print "Another step closer: ",$nextstate->Grade(),"\n";
  ...
  if (@result == 0)
  {
      my $p = MakePath($closesttogoalstate,0);
      push @result,$p;
      PrintPath($p,$doprint);
  }
...
sub MakePath
{
    ...
    return { ISSOLUTION=>$issolution,PATH=>$result};
}

sub PrintPath
{
    my ($po,$doprint) = @_;
    return unless $doprint;
    my $path = $po->{PATH};
    if ($po->{ISSOLUTION})
    {
    ...
    {
	print "No solution found, printing closest to goal.\n";
    }

    print $path->[0]->DisplayString();
    for (my $i = 1; $i < @$path ; ++$i)
    {
	print "Move: ",$path->[$i]->Move(),"\n";
	print $path->[$i]->DisplayString();
    }
}
